...
int main(void)
{
	...
	while(cin >> n >> m and (n or m))
	{
		...
		// ID of symbol.
        //first symbol id is 2 second is 3 and so on
		int id = 2;
		for(int i = 0; i < n; ++i)
		{
			for(int j = 0; j < m; ++j)
			{
				// If It is a 0 then increase the number of holes on the current id symbol and mark the holes with that id.
				if(grid[i][j] == 0)
				{ 
					++holes[grid[i][j - 1]];
					floodFill(i, j, grid[i][j - 1], 0);
				}
				// If It is a 1 then It is a new symbol so mark with new id.
				else if(grid[i][j] == 1)
					floodFill(i, j, id++, 1);
			}
		}
